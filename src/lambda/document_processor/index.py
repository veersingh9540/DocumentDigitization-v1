    ...
    try:
        ...
        for record in event['Records']:
            ...
            s3_client.put_object(
                ...
                Key=output_key,
                Body=json.dumps(document_data, default=str),  # Use default=str to handle non-serializable objects
            ...
            # For cylinder inventory, create specialized table entries
            if document_data.get('document_type') == 'cylinder_inventory':
                extracted_fields = document_data.get('extracted_fields', [])
                for record in extracted_fields:
                    # Structure the inventory data
                    inventory_data = {
                        'document_id': document_id,
                        'date': record.get('date'),
                        'month': record.get('month'),
                        'year': record.get('year'),
                        'opening_stock': record.get('values', [])[0] if len(record.get('values', [])) > 0 else None,
                        'receipt': record.get('values', [])[1] if len(record.get('values', [])) > 1 else None,
                        'total_stock': record.get('values', [])[2] if len(record.get('values', [])) > 2 else None,
                        'closing_stock': record.get('values', [])[3] if len(record.get('values', [])) > 3 else None,
                    }
                    db.insert_cylinder_inventory(inventory_data)
            else:
                # Insert extracted fields for other document types
